...
vim.keymap.set(
    ...
    { desc = '[r]eplace word with confirmation' }
)
...
--     vim.cmd 'w'
--     notify_action 'File saved'
-- end, vim.tbl_extend('force', opts, { desc = '[s]ave [f]ile' }))
...
vim.keymap.set(
    ...
    '<S-k>',
    ':wincmd k<CR>',
    vim.tbl_extend('force', opts, { desc = 'move to the window above' })
)
vim.keymap.set(
    'n',
    '<S-j>',
    ':wincmd j<CR>',
    vim.tbl_extend('force', opts, { desc = 'move to the window below' })
)
vim.keymap.set(
    'n',
    '<S-h>',
